...
输入（关键词/小说文本）
        ↓
   [Parser Agent]        ← 解析输入，提取人物/世界观
        ↓
   [Planner Agent]       ← 规划分支剧情树
        ↓
   [Writer Agent]        ← 写每个节点的对白/旁白
        ↓
   [Consistency Agent]   ← 检查前后一致性
        ↓
   [Gamification Agent]  ← 插入小游戏参数
        ↓
   [Direction Agent]     ← 分配演出档位
        ↓
   [Structurer Agent]    ← 打包成结构化 JSON
        ↓
    output/game.json
```
...
| 阶段 | 目标 | 交付 | 验证标准 |
| --- | --- | --- | --- |
| P0 MVP | 打通「小说改编 + 关键词原创 → 结构化 JSON → 引擎运行」主链路 | 七 Agent 流水线 + 引擎可跑的 demo | 能生成带 ≥3 分支、可玩的完整互动漫剧游戏（可视化游戏前的全部信息准备） |
| P1 可用 | 补齐可视化编辑器 + 数值模拟器 + 小游戏穿插 | 人机协作编辑 + 平衡性报告 | 支线相似度、分支到达率、结局分布达阈值（生成一个游戏demo） |
...
## 10. 【P0 MVP】安装与使用（Quick Start）
生成结果示例 JSON

```
output/game.json
├── 基本信息        标题、题材标签、一句话简介
├── 变量系统        好感度/勇气/金钱 等数值，初始值
├── 道具清单        每个道具的 ID、名称、描述
├── 分支剧情树      每个节点：
│   ├── 内容       对白/旁白/内心独白（干跑是模板句，接 LLM 后是真实文本）
│   ├── 选项       玩家能选什么、选了跳哪里、有什么前置条件
│   ├── 演出参数    背景图/BGM/CG/镜头/情绪强度/档位（静态/半动态/全视频）
│   ├── 道具操作    获得/消耗/需要哪些道具
│   └── 小游戏参数  类型/难度/时间窗口/成功失败跳转
├── 结局列表        每个结局的 ID 和触发条件
└── 素材需求清单    asset_prompt：告诉美术/AI绘图这里需要画什么
